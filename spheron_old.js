		...
		for(var key in inputSignals) {
			...
			for(var connection in this.io){
				if((this.io[connection]).id == key){
					...
					(this.io[connection]).lessonIdx = thisConnSignal.lessonIdx
				}
			}
		}
	}
	...
	for(var key in this.io) {
		...
		if(excludeThis == false){
			...
			for(var thisOutput in theseOutputs){
				...
				thisResults[theseOutputs[thisOutput]].lessonIdx = that.io[theseInputIdxs[0]].lessonIdx //<--- Why [0]???????
			}
...
Spheron.prototype.propagationMessageQueue = function(callback){
	var that = this;
	console.log(moduleName, 4, that.spheronId + '****activationQueue: ' + JSON.stringify(that.propagationMessageQueue))
